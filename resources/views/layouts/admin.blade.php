<!DOCTYPE html>
<html lang="id" x-data="{ dark: localStorage.getItem('theme') === 'dark', sidebarOpen: true }"
      x-init="dark ? $el.classList.add('dark') : $el.classList.remove('dark')"
      :class="{ 'dark': dark }"
      class="h-full">
<head>
    <meta charset="UTF-8">
...
<div class="flex h-screen overflow-hidden">
    ...
    <aside class="w-64 flex-shrink-0 flex flex-col relative"
        ...
        <div class="absolute inset-0 opacity-30 pointer-events-none"
             style="background: radial-gradient(ellipse at top left, rgba(167,139,250,0.4) 0%, transparent 60%)"></div>

        {{-- Logo --}}
        <div class="relative px-5 py-5 border-b border-white/10">
            <div class="flex items-center gap-3">
                <div class="w-11 h-11 rounded-xl bg-white/10 border border-white/20 flex items-center justify-center overflow-hidden flex-shrink-0">
        ...
        <nav class="relative flex-1 px-3 py-4 overflow-y-auto space-y-0.5">
            ...
            @if(auth()->user()->role->slug !== 'admin_fnb')
            {{-- Hotel --}}
            ...
            @if(auth()->user()->role->slug !== 'admin_hotel')
            {{-- F&B --}}
            <p class="px-3 pt-4 mb-2 text-[10px] font-semibold text-purple-300/60 uppercase tracking-widest">
                F&amp;B
            </p>
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        {{-- Topbar --}}
        <header class="flex-shrink-0 h-14 flex items-center justify-between px-6
            ...
            <div class="flex items-center gap-2">
                {{-- Dark mode toggle --}}
                <button @click="dark = !dark; dark ? $el.closest('html').classList.add('dark') : $el.closest('html').classList.remove('dark'); localStorage.setItem('theme', dark ? 'dark' : 'light')"
