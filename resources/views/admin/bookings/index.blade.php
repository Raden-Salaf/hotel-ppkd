...
{{-- Filter tabs --}}
<div class="flex items-center gap-1 mb-5 bg-white dark:bg-white/4 border border-gray-100 dark:border-white/8 rounded-2xl p-1.5 w-fit">
    ...
    <a href="{{ route('admin.bookings.index', $val ? ['status' => $val] : []) }}"
       class="px-4 py-1.5 rounded-xl text-xs font-medium transition-all duration-150
              {{ request('status') === $val || (request('status') === null && $val === '')
                 ? 'text-white shadow-sm' : 'text-gray-500 dark:text-white/40 hover:text-gray-700 dark:hover:text-white/70' }}"
       @if(request('status') === $val || (request('status') === null && $val === ''))
       style="background: linear-gradient(135deg, #7C3AED, #4F46E5)"
       @endif>
        {{ $label }}
    </a>
    ...
    <table class="w-full">
        <thead>
            <tr class="border-b border-gray-50 dark:border-white/5">
                <th class="px-5 py-3.5 text-left text-[11px] font-semibold text-gray-400 dark:text-white/30 uppercase tracking-wider">Kode</th>
                <th class="px-5 py-3.5 text-left text-[11px] font-semibold text-gray-400 dark:text-white/30 uppercase tracking-wider">Tamu</th>
                <th class="px-5 py-3.5 text-left text-[11px] font-semibold text-gray-400 dark:text-white/30 uppercase tracking-wider">Kamar</th>
                <th class="px-5 py-3.5 text-left text-[11px] font-semibold text-gray-400 dark:text-white/30 uppercase tracking-wider">Check-in</th>
                <th class="px-5 py-3.5 text-left text-[11px] font-semibold text-gray-400 dark:text-white/30 uppercase tracking-wider">Check-out</th>
                <th class="px-5 py-3.5 text-left text-[11px] font-semibold text-gray-400 dark:text-white/30 uppercase tracking-wider">Total</th>
                ...
                <td class="px-5 py-3.5">
                    <div class="flex items-center gap-2">
                        <a href="{{ route('admin.bookings.show', $booking) }}"
                           class="text-xs px-3 py-1.5 rounded-lg border border-gray-200 dark:border-white/10
